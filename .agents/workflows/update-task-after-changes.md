...
Mỗi khi đang thực hiện các mục trong task document (file `task.md` hoặc bất kỳ task artifact nào), sau khi sửa code xong **BẮT BUỘC** phải cập nhật lại task document:
...
3. **Sau khi sửa code xong**: Cập nhật task document ngay lập tức:
   ...
   - Thêm ghi chú thay đổi (ví dụ: "Đã thêm tính năng X", "Đã xóa hàm Y")
